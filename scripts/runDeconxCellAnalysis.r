...
tme <- tme_cluster(input = cell, features = colnames(cell)[2:65], 
                   id = "ID", scale = TRUE, method = "kmeans", 
                   min_nc =2, max.nc = 2)
colnames(tme) <- gsub(colnames(tme), pattern = "_xCell", replacement = "")

xcell_tme <- list('cell' = cell,
                  'tme' = tme)

save(xcell_tme, file = file.path(dir_out, 'xcell', 'xcell_tme.RData'))
################################################################
# Heatmap
################################################################
df <- tme[, -c(1,2)]
...
pdf(file=file.path(dir_out, "xcell", "heatmap_xcell.pdf"),
     width = 8, height = 8.5)
...
pdf(file=file.path(dir_out, "xcell/boxplot", paste(signature_name[i], "heatmap_xcell.pdf", sep="")),
     width = 4, height = 4)
